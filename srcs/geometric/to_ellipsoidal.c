...
/*   Updated: 2022/10/20 14:03:56 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	compute_height_and_lattitude(t_point point, t_ellipsoid ellipsoid,
				double *height, double *lat);
...
t_ellipsoidal_point	to_ellipsoidal(t_ellipsoid ellipsoid, t_point point)
{
	...
	compute_height_and_lattitude(point, ellipsoid, &el_point.height,
		&el_point.lattitude);
	el_point.lattitude += M_PI;
	return (el_point);
}

void	compute_height_and_lattitude(t_point point, t_ellipsoid ellipsoid,
			double *height, double *lat)
{
	double	e_2;
	double	p;
