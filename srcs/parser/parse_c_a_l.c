...
/*   Updated: 2022/09/21 18:19:53 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	parse_camera(char **args, t_parsing *parsing)
{
	...
	{
		ft_putendl_fd("Error\nImpossible to parse camera because argument's", 2);
	...
	{
		ft_putendl_fd("Error\nImpossible to parse camera because one comp", 2);
		ft_putendl_fd("osant of the orientation vector is not in [-1;1]", 2);
		free(obj);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse camera because FOV ", 2);
		ft_putendl_fd("is not in [0;180]", 2);
		free(obj);
		return (false);
	}
	parsing->camera = obj;
...
bool	parse_ambient_lightning(char **args, t_parsing *parsing)
{
	...
	{
		ft_putendl_fd("Error\nThere is more than one ambient ", 2);
		ft_putendl_fd("lightning in the scene", 2);
		return (false);
	}
	if (get_split_size(args) != 3)
	{
		ft_putendl_fd("Error\nImpossible to parse ambient light", 2);
		ft_putendl_fd("ning because argument's count is not 3", 2);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse ambient lightning ", 2);
		ft_putendl_fd("because lightning ration is not in [0;1]", 2);
		free(obj);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse ambient lightning becau", 2);
		ft_putendl_fd("se one of the color composant in not in [0;255]", 2);
		free(obj);
		return (false);
	}
