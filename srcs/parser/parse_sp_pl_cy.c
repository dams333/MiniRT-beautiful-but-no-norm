...
/*   Updated: 2022/09/26 12:58:58 by dhubleur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	parse_sphere(char **args, t_parsing *parsing)
{
	...
	{
		ft_putendl_fd("Error\nImpossible to parse sphere because ", 2);
		ft_putendl_fd("argument's count is not 4", 2);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse sphere because one of", 2);
		ft_putendl_fd(" the color composant in not in [0;255]", 2);
		free(obj);
		return (false);
	}
...
bool	parse_plane(char **args, t_parsing *parsing)
{
	...
	if (get_split_size(args) != 4)
	{
		ft_putendl_fd("Error\nImpossible to parse plane because argument's", 2);
	...
	{
		ft_putendl_fd("Error\nImpossible to parse plane because one ", 2);
		ft_putendl_fd("composant of the orientation vector ", 2);
		ft_putendl_fd("is not in [-1;1]", 2);
		free(obj);
		return (false);
	}
	...
	if (obj->color_r < 0 || obj->color_r > 255 || obj->color_g < 0
		|| obj->color_g > 255 || obj->color_b < 0 || obj->color_b > 255)
	{
		ft_putendl_fd("Error\nImpossible to parse plane because one of the", 2);
		ft_putendl_fd(" color composant in not in [0;255]", 2);
		free(obj);
		return (false);
	}
	if (!add_item_to_list(&(parsing->hittables), obj, CYLINDER))
...
bool	parse_cylinder(char **args, t_parsing *parsing)
{
	...
	{
		ft_putendl_fd("Error\nImpossible to parse cylinder because ", 2);
		ft_putendl_fd("argument's count is not 6", 2);
		return (false);
	}
	...
	if (obj->orientation_x < -1 || obj->orientation_x > 1
		...
		|| obj->orientation_z < -1 || obj->orientation_z > 1)
	{
		ft_putendl_fd("Error\nImpossible to parse cylinder because one ", 2);
		ft_putendl_fd("composant of the orientation vector ", 2);
		ft_putendl_fd("is not in [-1;1]", 2);
		free(obj);
		return (false);
	...
	if (obj->diameter < 0)
	{
		ft_putendl_fd("Error\nImpossible to parse cylinder ", 2);
		ft_putendl_fd("because diameter is < 0", 2);
		free(obj);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse cylinder ", 2);
		ft_putendl_fd("because height is < 0", 2);
		free(obj);
		return (false);
	}
	...
	{
		ft_putendl_fd("Error\nImpossible to parse cylinder because", 2);
		ft_putendl_fd(" one of the color composant in not in [0;255]", 2);
		free(obj);
		return (false);
	}
	if (!add_item_to_list(&(parsing->hittables), obj, CYLINDER))
